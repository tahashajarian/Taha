...
export function useChessController({ nodes, moveDuration, kingBRef }) {
  ...
  useEffect(() => {
    ...
    const sequence = [
      {
        key: "queen1",
        fn: () => queenApi.start({ position: [0, 0, -0.37], config: cfg }),
        feedback: { position: [0.23, 0.05, -0.27], delay: 150 },
      },
      { fn: () => pawnApi.start({ position: [0.12, 0, -0.2], config: cfg }), overlap: OVERLAP_PAWN },
      {
        key: "queenB",
        fn: () => queenBApi.start({ position: [0.1, 0, -0.1], config: cfg }),
        feedback: { position: [0.23, 0.05, -0.27], delay: 150 },
      },
      { fn: () => queenApi.start({ position: [0.1, 0, -0.4], config: cfg }), overlap: OVERLAP_QUEEN2 },
      {
        key: "knight",
        fn: () => {
    ...
    sequence.forEach((step, i) => {
      ...
      if (step.feedback && step.key) {
        ...
        timeoutsRef.current.push(
          setTimeout(() => pushFeedback(step.feedback.position, tex, FEEDBACK_MAP[step.key].duration), startAt + MOVE + step.feedback.delay)
        )
      }
